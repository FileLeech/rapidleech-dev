...
jQuery(oSettings.nPaginate).insertAfter(oSettings.nTable);jQuery(oSettings.nPrevious).click(function(){oSettings._iDisplayStart-=oSettings._iDisplayLength;
if(oSettings._iDisplayStart<0){oSettings._iDisplayStart=0}fnCallbackDraw(oSettings)
});jQuery(oSettings.nNext).click(function(){if(oSettings._iDisplayStart+oSettings._iDisplayLength<oSettings.fnRecordsDisplay()){oSettings._iDisplayStart+=oSettings._iDisplayLength
...
oSettings.nPaginate.appendChild(nNext);oSettings.nPaginate.appendChild(nLast);jQuery(nFirst).click(function(){oSettings._iDisplayStart=0;
fnCallbackDraw(oSettings)});jQuery(nPrevious).click(function(){oSettings._iDisplayStart-=oSettings._iDisplayLength;
if(oSettings._iDisplayStart<0){oSettings._iDisplayStart=0}fnCallbackDraw(oSettings)
});jQuery(nNext).click(function(){if(oSettings._iDisplayStart+oSettings._iDisplayLength<oSettings.fnRecordsDisplay()){oSettings._iDisplayStart+=oSettings._iDisplayLength
}fnCallbackDraw(oSettings)});jQuery(nLast).click(function(){var iPages=parseInt((oSettings.fnRecordsDisplay()-1)/oSettings._iDisplayLength,10)+1;
oSettings._iDisplayStart=(iPages-1)*oSettings._iDisplayLength;fnCallbackDraw(oSettings)
});jQuery("span",oSettings.nPaginate).bind("mousedown",function(){return false});jQuery("span",oSettings.nPaginate).bind("selectstart",function(){return false
...
jQuery("span",oSettings.nPaginateList).bind("mousedown",function(){return false});jQuery("span",oSettings.nPaginateList).bind("selectstart",function(){return false
});jQuery("span",oSettings.nPaginateList).click(function(){var iTarget=(this.innerHTML*1)-1;
...
}if(oSettings._iDisplayStart+oSettings._iDisplayLength>=oSettings.aiDisplay.length){oSettings._iDisplayStart-=oSettings._iDisplayLength;
...
nNewCell.innerHTML=sHtml;jQuery(nNewRow).insertAfter(nTr);if(!oSettings.oFeatures.bServerSide){oSettings.aoOpenRows.push({nTr:nNewRow,nParent:nTr})
...
}var nTrHead=jQuery("thead tr",oSettings.nTable)[0];var nTrFoot=jQuery("tfoot tr",oSettings.nTable)[0];
...
var oCol=oSettings.aoColumns[iLength];if(typeof oOptions.sType!="undefined"){oCol.sType=oOptions.sType;
...
_fnMap(oCol,oOptions,"bUseRendered");_fnMap(oCol,oOptions,"iDataSort")}oSettings.aoPreSearchCols[oSettings.aoPreSearchCols.length++]={sSearch:"",bEscapeRegex:true}
...
return iThisIndex}function _fnGatherData(oSettings){var nDataNodes;var iDataLength=jQuery("tbody:eq(0)>tr").length;
var iLoop;var i,j;if(oSettings.sAjaxSource===null){jQuery("tbody:eq(0)>tr",oSettings.nTable).each(function(){var iThisIndex=oSettings.aoData.length;
...
jQuery("td",this).each(function(i){aLocalData[i]=this.innerHTML})})}var iCorrector=0;for(i=0;
...
}nTh.innerHTML=oSettings.aoColumns[i].sTitle;nTr.appendChild(nTh)}}jQuery("thead",oSettings.nTable).html("")[0].appendChild(nTr)
}if(oSettings.oFeatures.bSort){for(i=0;i<oSettings.aoColumns.length;i++){jQuery(oSettings.aoColumns[i].nTh).click(function(e){var iDataIndex;
...
}jQuery("thead th",oSettings.nTable).mousedown(function(){this.onselectstart=function(){return false
...
jQuery(nRow).removeClass(oSettings.asStripClasses.join(" "));jQuery(nRow).addClass(oSettings.asStripClasses[iRowCount%iStrips]);
...
}if(typeof oSettings.fnHeaderCallback=="function"){oSettings.fnHeaderCallback(jQuery("thead tr",oSettings.nTable)[0],_fnGetDataMaster(oSettings),oSettings._iDisplayStart,oSettings._iDisplayEnd,oSettings.aiDisplay)
}if(typeof oSettings.fnFooterCallback=="function"){oSettings.fnFooterCallback(jQuery("tfoot tr",oSettings.nTable)[0],_fnGetDataMaster(oSettings),oSettings._iDisplayStart,oSettings._iDisplayEnd,oSettings.aiDisplay)
}var nTrs=jQuery("tbody:eq(0)>tr",oSettings.nTable);for(i=0;i<nTrs.length;i++){nTrs[i].parentNode.removeChild(nTrs[i])
}var nBody=jQuery("tbody:eq(0)",oSettings.nTable);for(i=0;i<anRows.length;i++){nBody[0].appendChild(anRows[i])
...
jQuery("input",nFilter).keyup(function(e){_fnFilterComplete(oSettings,{sSearch:this.value,bEscapeRegex:oSettings.oPreviousSearch.bEscapeRegex})
...
jQuery("select",nLength).change(function(e){oSettings._iDisplayLength=parseInt(jQuery(this).val(),10);
...
_fnDraw(oSettings)}}function _fnSortingClasses(oSettings){var i;var aaSort;var iColumns=oSettings.aoColumns.length;
for(i=0;i<iColumns;i++){jQuery(oSettings.aoColumns[i].nTh).removeClass("sorting_asc sorting_desc sorting")
...
break}}jQuery(oSettings.aoColumns[i].nTh).addClass(sClass)}}if(oSettings.oFeatures.bSortClasses){var nTrs=_fnGetTrNodes(oSettings);
jQuery("td",nTrs).removeClass("sorting_1 sorting_2 sorting_3");var iClass=1;for(i=0;i<aaSort.length;
i++){var iVis=_fnColumnIndexToVisible(oSettings,aaSort[i][0]);if(iVis!==null){if(iClass<=2){jQuery("td:eq("+iVis+")",nTrs).addClass("sorting_"+iClass)
}else{jQuery("td:eq("+iVis+")",nTrs).addClass("sorting_3")}iClass++}}}}function _fnVisibleToColumnIndex(oSettings,iMatch){var iColumn=-1;
...
var i;var oHeaders=jQuery("thead th",oSettings.nTable);for(i=0;i<iColums;i++){if(oSettings.aoColumns[i].bVisible){iVisibleColumns++;
...
sCalcHtml+="</tr>";nCalcTmp=jQuery(sTableTmp+sCalcHead+sCalcHtml+"</table>")[0];nCalcTmp.style.width=iTableWidth+"px";
nCalcTmp.style.visibility="hidden";nCalcTmp.style.position="absolute";oSettings.nTable.parentNode.appendChild(nCalcTmp);
var oNodes=jQuery("td",nCalcTmp);var iIndex;for(i=0;i<oNodes.length;i++){iIndex=oNodes[i].getAttribute("tag_index");
oSettings.aoColumns[iIndex].sWidth=jQuery("td",nCalcTmp)[i].offsetWidth+"px"}oSettings.nTable.parentNode.removeChild(nCalcTmp)
...
var iRowspan=nTds[j].getAttribute("rowspan")*1;if(!iColspan){iColumnShifted=fnShiftCol(aLayout,i,iColumn);
aLayout[i][iColumnShifted]=nTds[j];if(iRowspan){for(k=1;k<iRowspan;k++){fnAddRow(i+k);
aLayout[i+k][iColumnShifted]=ROWSPAN}}iColumn++}else{iColumnShifted=fnShiftCol(aLayout,i,iColumn);
...
}}}return aReturn}function _fnMap(oRet,oSrc,sName){if(typeof oSrc[sName]!="undefined"){oRet[sName]=oSrc[sName]
}}if(bApi){this.oApi._fnInitalise=_fnInitalise;this.oApi._fnLanguageProcess=_fnLanguageProcess;
...
this.oApi._fnDataToSearch=_fnDataToSearch;this.oApi._fnCalculateEnd=_fnCalculateEnd;
this.oApi._fnConvertToWidth=_fnConvertToWidth;this.oApi._fnCalculateColumnWidths=_fnCalculateColumnWidths;
this.oApi._fnArrayCmp=_fnArrayCmp;this.oApi._fnDetectType=_fnDetectType;this.oApi._fnGetDataMaster=_fnGetDataMaster;
...
_fnMap(oSettings,oInit,"sPaginationType");_fnMap(oSettings,oInit,"sAjaxSource");if(typeof oInit.sDom!="undefined"){oSettings.sDomPositioning=oInit.sDom
}if(typeof oInit.iDisplayLength!="undefined"){oSettings._iDisplayLength=oInit.iDisplayLength
}if(typeof oInit.aaData!="undefined"){bUsePassedData=true}if(typeof oInit.bStateSave!="undefined"){oSettings.oFeatures.bStateSave=oInit.bStateSave;
